...
#define DECLARE_MUTEX(mutex) CRITICAL_SECTION *mutex
#define INIT_MUTEX(mutex)                                       \
...
#define DECLARE_THREAD(t) std::thread *t; 
#define INIT_THREAD(t, threadEntryPoint, pointer) \
	t = new std::thread(threadEntryPoint, pointer);
#define JOIN_THREAD(t) t->join();
#define DELETE_THREAD(t) delete t;
...
class MegaWaiter : public ::mega::WinPhoneWaiter {};
...
{
public:
    MegaDbAccess(std::string *basePath = NULL) : SqliteDbAccess(basePath){}
};
...
class MegaNode
{
    public:
		...
		MegaNode(const char *name, int type, m_off_t size, ::mega::m_time_t ctime, ::mega::m_time_t mtime, ::mega::handle nodehandle, std::string *nodekey, std::string *attrstring);
...
struct MegaFileGet : public MegaFile
{
	...
	MegaFileGet(::mega::MegaClient *client, ::mega::Node* n, std::string dstPath);
	MegaFileGet(::mega::MegaClient *client, MegaNode* n, std::string dstPath);
	~MegaFileGet() {}
};
...
struct MegaFilePut : public MegaFile
{
	...
	MegaFilePut(::mega::MegaClient *client, std::string* clocalname, ::mega::handle ch, const char* ctargetuser);
    ~MegaFilePut() {}
};
...
class SearchTreeProcessor : public TreeProcessor
{
	...
	std::vector<::mega::Node *> &getResults();

    protected:
	const char *search;
	std::vector<::mega::Node *> results;
};
...
public:
    ...
    //Sync
	::mega::treestate_t syncPathState(std::string *path);
...
protected:
	...
	std::map<int, MegaRequest *> requestMap;
    std::map<int, MegaTransfer *> transferMap;
    int pendingUploads;
    int pendingDownloads;
    int totalUploads;
    int totalDownloads;
	std::set<MegaRequestListener *> requestListeners;
	std::set<MegaTransferListener *> transferListeners;
	std::set<MegaGlobalListener *> globalListeners;
	std::set<MegaListener *> listeners;
    bool waiting;
    bool waitingRequest;
    ...
    virtual void checkfile_result(::mega::handle h, ::mega::error e, byte* filekey, m_off_t size, ::mega::m_time_t ts, ::mega::m_time_t tm, std::string* filename, std::string* fingerprint, std::string* fileattrstring);
	...
	virtual void openfilelink_result(::mega::handle, const byte*, m_off_t, std::string*, const char*, ::mega::m_time_t, ::mega::m_time_t, int);
	...
	virtual void syncupdate_stuck(std::string*);
	...
	virtual void syncupdate_remote_move(std::string*, std::string*);
    virtual void syncupdate_treestate(::mega::LocalNode*);
	virtual bool sync_syncable(::mega::Node*);
	virtual bool sync_syncable(const char*name, std::string*, std::string*);
	...
	char *stringToArray(std::string &buffer);
