...
MegaSDK::MegaSDK(String^ basePath)
{
	std::string utf8basePath;
	MegaApi::utf16ToUtf8(basePath->Data(), basePath->Length(), &utf8basePath);
	if(basePath != nullptr) megaApi = new MegaApi(utf8basePath.data());
	else megaApi = new MegaApi();
	InitializeCriticalSectionEx(&listenerMutex, 0, 0);
}
...
{
	if (base64Handle == nullptr) return ::mega::UNDEF;
...
{
	if (handle == ::mega::UNDEF) return nullptr;
...
{
	if (node == nullptr) return ::mega::ACCESS_UNKNOWN;
